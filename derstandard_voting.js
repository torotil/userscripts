...
function replaceLink(e) {
  ...
  for (var i = snapResults.snapshotLength - 1; i >= 0; i--) {
	...
	// replace the link with a pair of "red" and "green" links
        elm.innerHTML = '<p style="font-weight:bold;"><a id="vd'+postID+'" class="onclick" '
                       +'style="color: rgb(207, 0, 0);">-</a>&nbsp;<a id="vn'+postID+'" class="onclick" '
                       +'style="color:blue;">0</a>&nbsp;'
                       +'<a id="vu'+postID+'" class="onclick" style="color: rgb(0, 154, 48);">'
                       +'+</a></p>';

	var votedown    = document.getElementById('vd'+postID);
	var voteneutral = document.getElementById('vn'+postID);
	var   voteup    = document.getElementById('vu'+postID);

	   votedown.addEventListener("click", function(e) {vote(e, 0);}, false); // 0 is "unnötig"
	voteneutral.addEventListener("click", function(e) {vote(e, 1);}, false); // 1 is "interessant"
...
if (unsafeWindow.document.observe) {
	unsafeWindow.document.observe('dom:loaded', replaceLink);
}
